            ...
            for center in response['centers']:
                for session in center['sessions']:
                    ...
                    if (session['min_age_limit'] == AGE_LIMT) & (session['available_capacity'] > 0) & (session['available_capacity_dose1'] > 0):
                        message_string=f"\nAvailable - {session['available_capacity_dose1']} in {center['name']} on {session['date']} for the age {session['min_age_limit']}+"
...
def book_appoinment(session_ids):
    ...
    for index, row in session_ids.iterrows():
        ...
        response_appoint, status = schedule_appointment(session_data, bearer_token, benficiaries_id_list, slot_preference=1)
        if status:
            send_push_notify(json.dumps(response_appoint))
            break
    return
